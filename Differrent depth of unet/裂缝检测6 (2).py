...
saver.restore(sess, "./Model/Model6_50epoch/model.ckpt")
...
for filesname in os.listdir(filePath_imgs):  # listdir的参数是文件夹的路径,listdir用于返回指定文件夹文件名字列表
    ...
    size = Img.shape
    print(size)
    after_mirror_h = size[1] + 210
    after_mirror_w = size[0] + 210
    mirror_img = np.zeros((after_mirror_w, after_mirror_h), dtype=int)
    mirror_img = cv2.copyMakeBorder(Img, 210, 210, 210, 210, cv2.BORDER_REFLECT)  # 轴对称扩充 相当于镜像填充
    ...
    for i in range(0, w):
        ...
        for j in range(0, h):
            ...
            img = mirror_img[i * S:(i + 1) * S+420, j * S:(j + 1) * S+420]  # 裁剪图像操作,是否可修改
