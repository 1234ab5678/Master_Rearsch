...
saver.restore(sess, "./Model/Model4_50epoch/model.ckpt")
...
for filesname in os.listdir(filePath_imgs):  # listdir的参数是文件夹的路径,listdir用于返回指定文件夹文件名字列表
    ...
    print("图像拼合开始")
    # 创建输出图片空矩阵
    out_end = np.zeros((size[0], size[1]), dtype=int)
    # 读取txt并得到输出矩阵
    num = 0
    out = np.zeros((out_size[0], out_size[1]), dtype=int)  # 见294，295行
    # print(out_w,out_h)
    d = int((out_w - S) / 2)
    # d = int((S-out_w) / 2)
    ...
    print(detection)
    np.savetxt(filePath_imgs + "detection" + ".txt", detection)
