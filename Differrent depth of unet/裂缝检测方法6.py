...
for filename in os.listdir(r'./picture/image/'):  # listdir的参数是文件夹的路径
    ...
    after_mirror_w = size[0] + 210#长宽各填充50个像素
    mirror_img = zeros((after_mirror_w, after_mirror_h), dtype=int)#
    mirror_img = cv2.copyMakeBorder(Img, 210,210,210,210, cv2.BORDER_REFLECT)  # 轴对称扩充 相当于镜像填充，给img上下左右各填充50，倒映填充
    ...
    for i in range(0, w):
        ...
        for j in range(0, h):
            ...
            img = mirror_img[i * S:(i+1) * S + 420, j * S:(j+1) * S +420]
