...
kernel_14=tf.Variable(initial_value=tf.random_normal(shape=[kernel_length,kernel_length,kernel_num,kernel_num],stddev=0.05))
bias_14=tf.Variable(initial_value=tf.random_normal(shape=[kernel_num],stddev=0.05))
kernel_15=tf.Variable(initial_value=tf.random_normal(shape=[kernel_length,kernel_length,kernel_num,2],stddev=0.05))
bias_15=tf.Variable(initial_value=tf.random_normal(shape=[2],stddev=0.05))
...
#最后一层，结束卷积
output_end=tf.nn.conv2d(input=output_14_active,filter=kernel_15,strides=[1,1,1,1],padding='VALID')+bias_15
...
regularization=tf.nn.l2_loss(kernel_1)+tf.nn.l2_loss(bias_1) + tf.nn.l2_loss(kernel_2)+tf.nn.l2_loss(bias_2) \
                ...
                +tf.nn.l2_loss(kernel_15) + tf.nn.l2_loss(bias_15)
...
path="E:/Bridgecrack_detection/dataset/crackdataset1"#基础路径
...
for epoch in range(5):
    for i in range(len(img_names)):
        ...
        if(i%10==0):
            ...
            #print(the_loss,end=' ')
            #学习率，实际上是优化算法的步长
            print('step:'+str(i+epoch*6130)+'\t'+'loss:'+str(the_loss)+'\t'+'learning_rate:'+str(learning_rate.eval()))
...
np.savetxt('./Model/Model4_5epoch/loss.txt',all_loss)

saver=tf.train.Saver()
saver.save(sess,"./Model/Model4_5epoch/model.ckpt")
